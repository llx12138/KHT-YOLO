...
class Albumentations:
    ...
    def __init__(self, p=1.0):
        ...
        try:
            ...
            # Transforms
            T = [
                # 对图像应用模糊效果，以减少细节并使模型更鲁棒
                A.Blur(p=0.1),  # 10%的几率应用模糊
                # 使用中值滤波去噪，特别有效于去除椒盐噪声
                A.MedianBlur(p=0.1),  # 10%的几率应用中值滤波
                # 将图像转换为灰度图像，增强模型对不同色彩的适应性
                A.ToGray(p=0.05),  # 5%的几率应用灰度转换
                # 应用对比度自适应直方图均衡化，增强图像的对比度
                A.CLAHE(p=0.1),  # 10%的几率应用CLAHE
                # 随机调整图像的亮度和对比度，提高模型对光照变化的鲁棒性
                A.RandomBrightnessContrast(p=0.2, brightness_limit=0.2, contrast_limit=0.2),  # 20%的几率，亮度和对比度调整范围为±20%
                # 随机调整图像的伽马值，进一步增强光照变化的鲁棒性
                A.RandomGamma(p=0.2, gamma_limit=(1.0, 1.5)),  # 20%的几率，伽马范围为0.5到1.5
                # 对图像进行压缩，模拟不同的图像质量，帮助模型适应低质量图像
                A.ImageCompression(quality_lower=50, p=0.1),  # 10%的几率，最低质量为50
                # 向图像中添加高斯噪声，增加训练数据的多样性
                A.GaussNoise(var_limit=(10.0, 50.0), p=0.2),  # 20%的几率，噪声方差范围为10到50
                # A.GaussNoise(var_limit=(10.0, 50.0), p=0.2),  # 20%的几率，噪声方差范围为10到50
            ]
