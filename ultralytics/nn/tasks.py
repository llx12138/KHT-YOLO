...
from ultralytics.nn.modules import (
    ...
    Segment,
    WorldDetect,
    v10Detect, GCSCA, GCSCA_Light, LocalContextShare,
)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            c2 = ch[f[-1]]
        # elif m in {GCSCA}:  # 自己加的注意力模块
        #     c1, c2 = ch[f], args[0]
        #     if c2 != nc:
        #         c2 = make_divisible(min(c2, max_channels) * width, 8)
        #     args = [c1, *args[1:]]
        elif m in {LocalContextShare, GCSCA, GCSCA_Light}:  # 支持 LocalContextShare, GCSCA, GCSCA_Light 模块
            c1, c2 = ch[f], args[0]  # 输入通道数为前一层的输出通道数，输出通道数为指定通道数

            if c2 != nc:  # 如果输出通道数不等于类别数
                c2 = make_divisible(min(c2, max_channels) * width, 8)  # 调整 c2，确保是 8 的倍数

            args = [c1, *args[1:]]  # 更新 args 以传递给模块
            ch.append(c2)  # 更新当前层的输出通道数到通道列表
        else:
            c2 = ch[f]
